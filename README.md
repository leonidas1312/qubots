...
Below is an example session demonstrating how to use the **Rastion** CLI to manage both optimizer and problem repos, then run them:

```bash
# 1. Create a new optimizer repository in the 'Rastion' org.
$ export GITHUB_TOKEN="<YOUR_PERSONAL_ACCESS_TOKEN>"
$ rastion create_repo my-optimizer --github-token $GITHUB_TOKEN
```

```bash
# 2. Push your optimizer code and config to GitHub.
$ cd /path/to/local/my-optimizer
$ ls
my_optimizer.py optimizer_config.json
$ rastion push_optimizer my-optimizer \
    --file my_optimizer.py \
...
$ rastion update_repo my-optimizer \
    --local-dir /path/to/local/my-optimizer \
    --branch main \
    --github-token $GITHUB_TOKEN
```
...
$ rastion clone_repo my-optimizer --branch main
```

```bash
# 7. Use 'run_optimizer' to test the optimizer with a problem from the hub.
$ rastion run_optimizer Rastion/my-optimizer-repo \
    --optimizer-rev main \
...
$ rastion delete_repo my-optimizer --github-token $GITHUB_TOKEN
```
